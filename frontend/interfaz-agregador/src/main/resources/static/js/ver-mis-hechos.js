                    ...
                    if (content.length !== 0) {
                        ...
                        content.forEach(hecho => {
                            const actionButton = hecho.visible
                                ? `<a href="/hechos/${hecho.id}" id="btn-editar-hecho-${hecho.id}" data-hecho='${JSON.stringify(hecho)}' class="btn-ver-hecho flex gap-2 align-items-center">
                            ...
                            html += `
                                <div class="list-group-item">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h4 class="texto-clamp font-medium text-gray-900 mb-0.5 max-w-[302.37px]">${hecho.titulo}</h4>
                                        ${actionButton}
                                    </div>
                                    <p class="texto-clamp">${hecho.descripcion}</p>
                                    <small>Fecha de carga: ${new Date(hecho.fechaCarga).toLocaleDateString()}</small>
                                </div>`
                        });
